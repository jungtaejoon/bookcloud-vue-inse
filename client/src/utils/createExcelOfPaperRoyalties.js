...
const createExcelOfPaperRoyalties = async (author, quarter, authorRoyalties) => {
    ...
    worksheet.getCell("E11").value = "정가(a)";
    worksheet.getCell("G11").value = "인세율(b)";
    worksheet.getCell("I11").value = "판매부수(c)*";
    worksheet.getCell("K11").value = "인세(d)";
    ...
    const settlementTitleRow = worksheet.addRow(["도서명", "", "ISBN", "", "소득세(e)", "", "지방소득세(f)", "", "선인세 및 기타(g)", "", "실수령액**"])
    const settlementTitleRow2 = worksheet.addRow(["", "", "", "", "d * 3%", "", "e * 10%", "", "*이월 금액 포함", "", "d - e - f - g"]);
